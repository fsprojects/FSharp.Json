...
Each feature of FSharp.Json is thoroughly covered by [unit tests](tree/master/tests/FSharp.Json.Tests).

## API Overview

Most of API functions are defined in [Json module](tree/master/src/FSharp.Json/Interface.fs).
...
Prefix `Ex` stands for "extended". Both of these functions take [JsonConfig](tree/master/src/FSharp.Json/InterfaceTypes.fs) instance as a first parameter.

#### Configuration

[JsonConfig](tree/master/src/FSharp.Json/InterfaceTypes.fs) represents global configuration of serialization.
...
The function is a shorthand to using `unformatted` member on [JsonConfig](tree/master/src/FSharp.Json/InterfaceTypes.fs).
...
EnumValue member of [JsonField](tree/master/src/FSharp.Json/InterfaceTypes.fs) attribute could be used to change serialization of enums.
There are two [modes](tree/master/src/FSharp.Json/InterfaceTypes.fs) supported currently: enum value name and enum value.
...
The string that represents union case key could be changed with [JsonUnionCase attribute](tree/master/src/FSharp.Json/InterfaceTypes.fs).
...
There's [union mode](tree/master/src/FSharp.Json/InterfaceTypes.fs) that represents union as JSON object with two fields.
One field is for case key and another one is for case value. This mode is called "case key as a field value"
If this mode is used then names of these two field should be provided through [JsonUnion attribute](tree/master/src/FSharp.Json/InterfaceTypes.fs).
...
[Type Transform](tree/master/src/FSharp.Json/InterfaceTypes.fs) is applied in the middle of this translation: F# Type -> Alternative F# Type -> JSON type.
...
The [Transforms](tree/master/src/FSharp.Json/Transforms.fs) module contains transforms that are defined by FSharp.Json library.
You can define your own transforms by implementing [ITypeTransform interface](tree/master/src/FSharp.Json/InterfaceTypes.fs).
...
In such case [DateTimeEpoch transform](tree/master/src/FSharp.Json/Transforms.fs) is useful.
...
For allowing obj type in serialization/deserialization allowUntyped flag should be set to `true` on [JsonConfig](tree/master/src/FSharp.Json/InterfaceTypes.fs).
