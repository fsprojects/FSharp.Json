            ...
            | 1 ->
                ...
                let values =
                    ...
                    | 1 ->
                        ...
                        let propValue =
                            deserializeUnwrapOption path propType fieldAttr (Some jvalue)
                ...
                | JsonValue.Record fields ->
                    let fieldName, fieldValue =
                        ...
                        | UnionMode.CaseKeyAsFieldValue ->
                            if fields.Length < 2 then
                                failDeserialization path
                                <| sprintf
                                    "Failed to parse union from record with %i fields, should be at least 2 fields."
                                    fields.Length

                            let caseKeyField =
                                fields
                                |> Seq.tryFind (fun f -> fst f = jsonUnion.CaseKeyField)
                            ...
                            let caseKeyField =
                                ...
                                | None ->
                                    failDeserialization path
                                    <| sprintf
                                        "Failed to parse union, unable to find union case field: %s."
                                        jsonUnion.CaseKeyField

                            let caseValueField =
                                fields
                                |> Seq.tryFind (fun f -> fst f = jsonUnion.CaseValueField)
                            ...
                            let caseValueField =
                                ...
                                | None ->
                                    failDeserialization path
                                    <| sprintf
                                        "Failed to parse union, unable to find union case field value: %s."
                                        jsonUnion.CaseValueField
                            ...
                            let caseName =
                                JsonValueHelpers.getString caseNamePath (snd caseKeyField)

                            (caseName, snd caseValueField)
                        | mode ->
                            failDeserialization path
                            <| sprintf "Failed to parse union, unsupported union mode: %A" mode

                    let casePath =
                        JsonPathItem.Field fieldName |> path.createNew
                    ...
                    let values =
                        ...
                        | 1 ->
                            ...
                            let propValue =
                                deserializeUnwrapOption casePath propType fieldAttr (Some fieldValue)
