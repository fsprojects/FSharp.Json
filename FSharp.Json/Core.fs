...
module internal Core =
    ...
    let getJsonUnionCaseName
        (config: JsonConfig)
        (jsonUnion: JsonUnion)
        (jsonUnionCase: JsonUnionCase)
    ...
    let rec serialize (config: JsonConfig) (t: Type) (value: obj) : JsonValue =
        let serializeEnum (t: Type) (jsonField: JsonField) (value: obj) : JsonValue =
            ...
            | mode ->
                failSerialization
                <| sprintf "Failed to serialize enum %s, unsupported enum mode: %A" t.Name mode
        ...
        let getUntypedType (t: Type) (value: obj) : Type =
            if t = typeof<obj> then
                if config.allowUntyped then
                    ...
                    failSerialization
                    <| "Failed to serialize untyped data, allowUntyped set to false"
            else
                t
        ...
        let serializeNonOption (t: Type) (jsonField: JsonField) (value: obj) : JsonValue =
            ...
            | false ->
                let t, value =
                    transformToTargetType t value jsonField.Transform

                let t = getUntypedType t value

                match t with
                | t when t = typeof<uint16> -> JsonValue.Number(decimal (value :?> uint16))
                | t when t = typeof<int16> -> JsonValue.Number(decimal (value :?> int16))
                | t when t = typeof<int> -> JsonValue.Number(decimal (value :?> int))
                | t when t = typeof<uint32> -> JsonValue.Number(decimal (value :?> uint32))
                | t when t = typeof<int64> -> JsonValue.Number(decimal (value :?> int64))
                | t when t = typeof<uint64> -> JsonValue.Number(decimal (value :?> uint64))
                | t when t = typeof<bigint> -> JsonValue.Number(decimal (value :?> bigint))
                | t when t = typeof<single> -> JsonValue.Float(float (value :?> single))
                | t when t = typeof<float> -> JsonValue.Float(value :?> float)
                | t when t = typeof<decimal> -> JsonValue.Number(value :?> decimal)
                | t when t = typeof<byte> -> JsonValue.Number(decimal (value :?> byte))
                | t when t = typeof<sbyte> -> JsonValue.Number(decimal (value :?> sbyte))
        ...
        let serializeProperty (therec: obj) (prop: PropertyInfo) : (string * JsonValue) option =
            ...
            let jvalue =
                serializeUnwrapOption prop.PropertyType jsonField propValue
        ...
        let serializeTupleItems (types: Type seq) (values: IEnumerable) : JsonValue =
            let items =
                values.Cast<Object>()
                |> Seq.zip types
                |> Seq.map (fun (t, value) -> serializeUnwrapOption t JsonField.Default value)
                |> Seq.map (Option.defaultValue JsonValue.Null)
        ...
        let serializeUnion (t: Type) (theunion: obj) : JsonValue =
            ...
            let theCase =
                getJsonUnionCaseName config jsonUnion jsonUnionCase caseInfo
            ...
            | _ ->
                ...
                let jvalue =
                    ...
                    | _ -> serializeTupleItems types values
                ...
                | _ ->
                    ...
                    | UnionMode.CaseKeyAsFieldValue ->
                        let jkey =
                            (jsonUnion.CaseKeyField, JsonValue.String theCase)
                    ...
                    | UnionMode.CaseKeyDiscriminatorField ->
                        ...
                        | JsonValue.Record jrecord ->
                            JsonValue.Record(
                                Array.append [| (jsonUnion.CaseKeyField, JsonValue.String theCase) |] jrecord
                            )
                        | _ ->
                            failSerialization
                            <| sprintf
                                "Failed to serialize union, union mode %A supports only objects as discriminated field could exist only in object"
                                jsonUnion.Mode
                    | mode ->
                        failSerialization
                        <| sprintf "Failed to serialize union, unsupported union mode: %A" mode
    ...
    let failDeserialization (path: JsonPath) (message: string) =
        let message =
            sprintf "JSON Path: %s. %s" (path.toString ()) message
    ...
    let rec deserialize (config: JsonConfig) (path: JsonPath) (t: Type) (jvalue: JsonValue) : obj =
        let deserializeEnum (path: JsonPath) (t: Type) (jsonField: JsonField) (jvalue: JsonValue) : obj =
            ...
            | mode ->
                ...
                <| sprintf "Failed to deserialize enum %s, unsupported enum mode: %A" t.Name mode
        ...
        let getUntypedType (path: JsonPath) (t: Type) (jvalue: JsonValue) : Type =
            ...
            | t when t = typeof<obj> ->
                if config.allowUntyped then
                    ...
                    failDeserialization path
                    <| sprintf "Failed to deserialize object, allowUntyped set to false"
            | t -> t
        ...
        let deserializeNonOption (path: JsonPath) (t: Type) (jsonField: JsonField) (jvalue: JsonValue) : obj =
            ...
            | false ->
                ...
                let jvalue =
                    match t with
                    | t when isNull t -> null :> obj
                    | t when t = typeof<int16> -> JsonValueHelpers.getInt16 path jvalue :> obj
                    | t when t = typeof<uint16> -> JsonValueHelpers.getUInt16 path jvalue :> obj
                    | t when t = typeof<int> -> JsonValueHelpers.getInt path jvalue :> obj
                    | t when t = typeof<uint32> -> JsonValueHelpers.getUInt32 path jvalue :> obj
                    | t when t = typeof<int64> -> JsonValueHelpers.getInt64 path jvalue :> obj
                    | t when t = typeof<uint64> -> JsonValueHelpers.getUInt64 path jvalue :> obj
                    | t when t = typeof<bigint> -> JsonValueHelpers.getBigint path jvalue :> obj
                    | t when t = typeof<single> -> JsonValueHelpers.getSingle path jvalue :> obj
                    | t when t = typeof<float> -> JsonValueHelpers.getFloat path jvalue :> obj
                    | t when t = typeof<decimal> -> JsonValueHelpers.getDecimal path jvalue :> obj
                    | t when t = typeof<byte> -> JsonValueHelpers.getByte path jvalue :> obj
                    | t when t = typeof<sbyte> -> JsonValueHelpers.getSByte path jvalue :> obj
                    | t when t = typeof<bool> -> JsonValueHelpers.getBool path jvalue :> obj
                    | t when t = typeof<string> -> JsonValueHelpers.getString path jvalue :> obj
                    | t when t = typeof<char> -> JsonValueHelpers.getChar path jvalue :> obj
                    | t when t = typeof<DateTime> ->
                    ...
                    | t when t = typeof<Guid> -> JsonValueHelpers.getGuid path jvalue :> obj
                    | t when t.IsEnum -> deserializeEnum path t jsonField jvalue
                    | t when
                        isTuple t
                        || isList t
                        || isArray t
            ...
            | t when isOption t ->
                ...
                | None ->
                    ...
                    | RequireNull ->
                        failDeserialization
                            path
                            "Option field is not found while using RequireNull option deserialization"
                    | AllowOmit -> optionNone t
            | _ ->
                match jvalue with
                | Some jvalue -> deserializeNonOption path t jsonField jvalue
                | None ->
        ...
        let deserializeArray (path: JsonPath) (t: Type) (jvalue: JsonValue) : obj =
            ...
            | JsonValue.Array jvalues ->
                ...
                let arr =
                    Array.CreateInstance(itemType, arrayValues.Length)
        ...
        let deserializeTupleElements (path: JsonPath) (types: Type []) (jvalue: JsonValue) : obj [] =
            ...
            | JsonValue.Array values ->
                if types.Length <> values.Length then
                    failDeserialization
                        path
                        "Failed to parse tuple. Number of values in JSON list does not match number of elements in tuple."
        ...
        let deserializeTuple (path: JsonPath) (t: Type) (jvalue: JsonValue) : obj =
            ...
            let tupleValues =
                deserializeTupleElements path types jvalue
        ...
        let deserializeProperty (path: JsonPath) (fields: (string * JsonValue) array) (prop: PropertyInfo) : obj =
            ...
            let field =
                fields |> Seq.tryFind (fun f -> fst f = name)

            let fieldValue = field |> Option.map snd

            let propPath =
                JsonPathItem.Field name |> path.createNew
        ...
        let getUnionCaseInfo (path: JsonPath) (t: Type) (jCaseName: string) : UnionCaseInfo =
            ...
            let caseInfo =
                unionCases
                |> Array.tryFind (fun c -> getJsonUnionCaseName config jsonUnion (getJsonUnionCase c) c = jCaseName)
        ...
        let mustFindField (path: JsonPath) (fieldName: string) (fields: (string * JsonValue) []) : string * JsonValue =
            let caseKeyField =
                fields |> Seq.tryFind (fun f -> fst f = fieldName)

            match caseKeyField with
            | Some field -> field
            | None ->
                failDeserialization path
                <| sprintf "Failed to parse union, unable to find field: %s." fieldName
        ...
        let makeUnion (path: JsonPath) (t: Type) (jCaseName: string) (jCaseValue: JsonValue) : obj =
            ...
            let casePath =
                JsonPathItem.Field jCaseName |> path.createNew
        ...
        let deserializeUnion (path: JsonPath) (t: Type) (jvalue: JsonValue) : obj =
            ...
            | [| caseInfo |] ->
                ...
                let values =
                    match props with
                    | [| prop |] -> [| deserializeUnwrapOption path prop.PropertyType fieldAttr (Some jvalue) |]
                    | _ ->
                        let propsTypes =
                            props |> Array.map (fun p -> p.PropertyType)
            ...
            | _ ->
                match jvalue with
                | JsonValue.String caseName -> FSharpValue.MakeUnion(caseName |> getUnionCaseInfo path t, null)
                | JsonValue.Record fields ->
                    match jsonUnion.Mode with
                    | UnionMode.CaseKeyDiscriminatorField ->
                    ...
                    | UnionMode.CaseKeyAsFieldValue ->
                        ...
                        let _, jCaseValue =
                            mustFindField path jsonUnion.CaseValueField fields
                    ...
                    | UnionMode.CaseKeyAsFieldName ->
                        match fields with
                        | [| (jCaseName, jCaseValue) |] -> makeUnion path t jCaseName jCaseValue
                        | _ ->
                            failDeserialization path
                            <| sprintf
                                "Failed to parse union from record with %i fields, should be 1 field."
                                fields.Length
