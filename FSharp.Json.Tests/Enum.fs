...
module Enum =
    ...
    type TheNumberEnum = { value: NumberEnum }

    [<Test>]
    let ``Enum serialization/deserialization`` () =
        let expected =
            { TheNumberEnum.value = NumberEnum.Three }
    ...
    let ``Enum serialization - config setting`` () =
        ...
        let config =
            JsonConfig.create (unformatted = true, enumValue = EnumMode.Value)
    ...
    let ``Letter Enum value deserialization`` () =
        let expected =
            { TheAttributedLetterEnum.value = LetterEnum.LetterB }
    ...
    let ``Letter Enum value serialization`` () =
        let value =
            { TheAttributedLetterEnum.value = LetterEnum.LetterC }
    ...
    let ``Number Enum value deserialization`` () =
        let expected =
            { TheAttributedNumberEnum.value = NumberEnum.Two }
